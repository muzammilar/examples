...
CREATE TABLE IF NOT EXISTS test.test_table_hourly_unreplicated_smt_local ON CLUSTER replicated_with_keeper
(
    `insert_timestamp` DateTime,
    `val` UInt64,
    `total` UInt64
...
CREATE MATERIALIZED VIEW IF NOT EXISTS test_mvs.test_table_hourly_unreplicated_smt_mv_local ON CLUSTER replicated_with_keeper TO test.test_table_hourly_unreplicated_smt_local AS
SELECT
    toStartOfHour(insert_timestamp) AS insert_timestamp,
    val,
    count() as total
